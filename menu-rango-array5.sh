...
while true; do
    ...
    5)
        ...
        # Crear array asociativo para contar los números generados aleatoriamente
        declare -A numeros_repetidos
        for ((i = 1; i <= cantidad; i++)); do
            num=$((RANDOM % 37))
            echo "Número aleatorio $i: $num"
            verificar_rango $num 0 36
            ((numeros_repetidos[$num]++)) # Incrementar la cantidad de veces que aparece este número
        done
        # Mostrar la cantidad de veces que aparece cada número
        echo "Cantidad de veces que aparecen los números generados aleatoriamente:"
        for num in "${!numeros_repetidos[@]}"; do
            echo "$num: ${numeros_repetidos[$num]}"
        done
        ;;
    4) exit ;;
...
# Para contar la cantidad de números repetidos generados aleatoriamente, puedes utilizar un array asociativo en el que cada elemento corresponde a un número y su valor es la cantidad de veces que aparece en la lista. Puedes incrementar el valor de cada elemento cada vez que aparezca el número correspondiente en la lista generada.